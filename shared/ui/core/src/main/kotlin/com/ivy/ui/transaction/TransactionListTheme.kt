...
internal object TransactionListTheme {
    val colors: TransactionListColors
        @Composable
        @ReadOnlyComposable
        get() = transactionListColors(
            theme = LocalThemeState.current.theme,
            isDarkModeEnabled = isSystemInDarkTheme(),
        )

    val typo: TransactionListTypography = transactionListTypography()
    val shapes: TransactionListShapes = transactionListShapes()
}

internal interface TransactionListColors {
    val pure: Color
...
internal interface TransactionListTypography {
    val b1: TextStyle
    val b2: TextStyle
    val c: TextStyle
    val nB2: TextStyle
...
internal interface TransactionListShapes {
...
private fun transactionListColors(
    theme: Theme,
    isDarkModeEnabled: Boolean,
): TransactionListColors {
    return when (theme) {
        Theme.LIGHT -> object : TransactionListColors {
        ...
        Theme.DARK -> object : TransactionListColors {
        ...
        Theme.AMOLED_DARK -> object : TransactionListColors {
        ...
        Theme.AUTO -> if (isDarkModeEnabled) {
            transactionListColors(Theme.DARK, true)
        } else {
            transactionListColors(Theme.LIGHT, false)
        }
    }
}

private fun transactionListTypography(): TransactionListTypography {
    ...
    return object : TransactionListTypography {
...
private fun transactionListShapes(): TransactionListShapes {
    return object : TransactionListShapes {
