...
在保持原版记账体验的基础上，按个人使用习惯陆续做了一些增强：

- **全局批量修改**：按月份、类别、标签筛选出一批交易并预览，再把它们的类别、账户或标签统一改成新值，省去逐笔修改。
- **GitHub 私有仓库云备份**：在设置里填好个人 access token 和仓库地址，一键把全部数据备份到自己的私有仓库，需要时再一键恢复。
- **搜索页收支汇总**：搜索结果上方实时显示当前匹配交易的收入与支出合计，随输入即时变化。
- **账户交易筛选**：进入某个账户的交易明细后可按类别和标签筛选，上方统计同步更新，交易条目也会显示标签。
- **更顺手的时间切换**：首页月份块两侧加了左右箭头快捷切换月份；类别页同样支持按月份查看各类别开销。
- **主页上部重构**：月份切换移到左上角并保留左右箭头，搜索与批量修改改为顶栏图标直达；移除原「快捷访问」下拉菜单与储蓄目标，收支概览卡改为更紧凑的净额 + 走势 + 收入/支出小卡。
- **账户与分类整合**：分类归属到账户，分类页并入账户页统一查看与编辑，记账时的分类选择器按所选账户过滤；账户支持长按拖拽排序，新建账户/分类自动分配不重复的醒目颜色。
- **界面精简**：固定为中文 + 人民币（移除多语言与多货币切换），并从界面下线预算、借贷、计划付款入口（数据层与备份保留，可随时恢复）；「我的」页更名为「设置」。
- **报告页重构**：打开即显示当月交易，用页面内 chips 逐步筛选类型、类别、账户和标签；支持按金额排序、高级筛选（关键词与金额范围滑动条），统计条显示收入、支出与净额。
...
- `feature/wallet` — 核心记账与编辑：首页、账户、分类、交易、搜索
