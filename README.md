...
- 继续清理 feature 的 Gradle 依赖：`:feature:search`、`:feature:piechart` 和 `:feature:main` 已去掉对 `shared:data:core` 的直接依赖；其中 search/main 只补充所需的 `shared:data:model`，settings 因仍直接使用备份恢复用例暂时保留 data core。
