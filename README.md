...
- 基础资源图标绘制开始迁入 `shared:ui:core` 的 `ResourceIcon`；编辑交易、计划付款编辑与列表、预算、自定义汇率卡片、首页和主底栏中的描述、日期、重复规则、交易类型切换、底部账户表单、状态和入口图标已脱离 legacy `IvyIcon`，并在调用处显式传入原主题文字色、选中态颜色或业务状态色。
