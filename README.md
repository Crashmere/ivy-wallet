...
1. 继续替换旧设计兼容 API：下一步优先处理 `shared:ui:legacy` 中剩余的旧 Wallet UI 包名，例如 `com.ivy.wallet.ui.edit.core`。
2. 收敛旧编辑交易底部表单组件，把通用部分迁到更清晰的 legacy/ui 包名，功能专用部分再下沉到对应 feature。
