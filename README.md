...
- 借贷详情页继续收窄内部导航职责：Header 关闭按钮、贷款账户按钮和还款记录账户按钮不再直接调用 `navigation()`，统一通过页面入口传入回调执行返回或进入账户交易筛选。
...
- 编辑交易页继续收窄内部导航职责：UI 主体不再直接调用 `navigation()`，工具栏关闭和“添加计划付款”入口改为普通回调，由 `EditTransactionScreen` 页面入口统一执行返回和计划付款 route 构造。
...
- 设置页主体不再直接调用 `navigation()` 或构造恢复导入、汇率页面 route；返回、进入导入和进入汇率都改为由 `SettingsScreen` 页面入口传入回调处理。
...
- 旧交易列表公开模型继续收窄：分类列表从完整 `Category` 改为只含 id、名称、颜色和图标的 `TransactionListCategory`；首页、搜索、报表和交易页在各自入口完成展示模型映射。
- 旧交易列表到期/逾期 section 输入继续收窄：删除独立 `DueSection` 旧模型，改用只含交易列表、展开状态、收入和支出数字的 `TransactionListDueSection`，不再要求调用端构造完整 `IncomeExpensePair`。
...
- 旧标签弹窗选中状态继续收窄：`ShowTagModal` 的选中列表和选择/删除回调改用普通 `UUID`，`TagId` 包装只留在编辑交易和报表筛选的业务事件边界。
...
- 基础资源图标绘制开始迁入 `shared:ui:core` 的 `ResourceIcon`；编辑交易、计划付款编辑与列表、预算、自定义汇率卡片、首页、主底栏、报表、饼图、分类和借贷中的描述、日期、重复规则、交易类型切换、底部账户表单、状态、筛选、入口、空状态和 tab 图标已脱离 legacy `IvyIcon`，并在调用处显式传入原主题文字色、选中态颜色或业务状态色。
...
- 旧底部栏渐隐背景 helper 已从 `shared:ui:legacy` 迁到 `shared:ui:core` 的 Compose 工具包；预算、分类、汇率、饼图统计、计划付款和旧弹窗继续保留原渐隐背景行为，但不再通过 legacy theme 导入通用 Modifier。
- 删除旧主题中已经没有调用方的底部渐隐背景 helper；当前渐隐背景只保留仍在使用的 `shared:ui:core` 顶部 helper 和页面私有实现。
...
- 收窄弹窗未选中态旧灰色依赖：计划付款重复规则弹窗和借贷弹窗不再直接导入旧 `Gray` 常量，未选中选项和辅助日期文案改用当前主题灰色。
...
- 收窄金额输入内部 helper：金额表达式规范化、本地小数分隔符、整数金额格式化和金额输入偏好状态转换改为 `shared:ui:core` 内部 API，外部继续使用正式格式化函数、金额弹窗和偏好模型。
- 收窄弹窗主按钮公开面：`ModalSet` 和 `ModalSkip` 只服务 UI core 内部弹窗，改为内部 API；删除没有调用方的旧 `ModalDelete` 包装，feature 继续使用各自私有删除按钮或 `DeleteModal`。
...
- 收窄首页和钱包统计账户边界：首页账户列表和钱包收入/支出统计改用正式 `Account` 模型，钱包余额计算也不再经由 `GetLegacyAccountsUseCase` 做二次转换。
...
- 收窄报表页账户缓存边界：报表 ViewModel 改为缓存正式 `Account`，筛选 UI 使用本模块轻量 `ReportAccount`，金额过滤、CSV 导出和旧收入/支出统计入口才局部转换为 `LegacyAccount`。
- 收窄饼图统计账户读取边界：饼图数据构建改用正式 `GetAccountsUseCase`，账户筛选和默认纳入余额判断基于正式 `Account` 完成，旧收入/支出和分类金额算法入口才局部转换为 `LegacyAccount`。
...
- 收窄首页计划付款操作边界：首页到期交易列表暂时继续向旧 UI 组件提供旧展示模型，但支付/跳过操作改为通过交易 ID 调用正式计划付款用例；首页 ViewModel 不再为了操作到期交易而查找或传递完整 `LegacyTransaction`。
- 收窄交易列表和报表计划付款操作边界：两个页面的到期交易列表仍暂时提供旧展示模型，但支付/跳过操作同样改为通过交易 ID 调用正式计划付款用例；无调用方的批量旧计划付款操作 use case 已删除。
- 收窄编辑交易页计划付款操作边界：计划交易支付不再接收完整旧交易模型写库，改为通过交易 ID 调用正式计划付款用例；支付成功后只重新读取写库结果同步旧编辑页本地展示状态，旧单条计划付款操作 use case 已删除。
- 收窄交易列表账户历史边界：账户筛选页的历史列表不再先把正式交易批量转成带标签的旧交易模型，而是直接使用正式 `BuildTransactionHistoryItemsUseCase` 构建历史项；无调用方的带标签旧交易映射 use case 已删除。
- 收窄交易列表账户转账边界：报表进入的账户转账虚拟分类不再用旧交易模型构建历史或计算转账收支，改为基于正式交易列表调用正式历史构建和收支统计用例；无调用方的 `BuildLegacyTransactionHistoryItemsUseCase` 已删除。
- 收窄饼图统计输入边界：饼图页从 route 交易 ID 还原输入时不再先转旧交易模型，饼图构建用例改为接收正式交易列表；总额/转账收支统计改用正式交易统计用例，分类金额算法暂时只在用例内部局部转换旧模型。
...
- 清空 `shared:domain` 的 legacy 模型依赖：历史列表新增正式 `TransactionHistoryTransaction`，domain 构建历史项时返回正式交易和正式标签；旧交易卡片需要的 `LegacyTransaction` 适配已下沉到 `shared:ui:legacy`，`LegacyTransaction` 不再实现 `TransactionHistoryItem`。
...
- 缓冲金额、借贷、借贷记录和计划付款重复规则弹窗的 `*ModalData` 已归回 `shared:ui:legacy`；这些数据对象本质上仍是旧弹窗入参，不再伪装成 UI core 公共 API。账户、分类和周期弹窗后续已改为显式参数，不再公开对应 data class。
...
- 编辑交易和计划付款编辑页的底部金额/账户面板已继续收窄为模块内部入口；外部仍只通过 `EditTransactionScreen` 和 `EditPlannedScreen` 两个页面入口进入对应编辑流程。
...
- 计划付款列表页的账户展示数据已改为从正式 `Account` 模型映射成本 feature 私有的 `PlannedPaymentAccount`，不再为了列表卡片展示注入 `GetLegacyAccountsUseCase`。
...
- 报表页和报表筛选浮层改用报表模块私有复选行；账户弹窗的复选行也已移回 `AccountModal` 文件私有实现，`shared:ui:legacy` 删除不再作为通用组件使用的 `IvyCheckbox` 文件；借贷弹窗迁回 feature 后使用借贷模块私有复选行。
...
- 报表页 UI 状态和筛选浮层中的账户已收窄为 `ReportAccount` 轻量模型；完整 `LegacyAccount` 只保留在 ViewModel 内部用于金额筛选、CSV 导出和现有 legacy 换算逻辑。
...
- 旧渐变按钮和描边胶囊按钮的底层绘制已迁入 `shared:ui:core` 的 `GradientButton` / `OutlinedPillButton`；`shared:ui:legacy` 暂时只保留 `IvyButton` / `IvyOutlinedButton` 兼容包装来填入旧主题默认值。
- 预算、分类、汇率和饼图统计页底部栏的新增按钮，以及分类排序弹窗的确认按钮，已直接改用 `shared:ui:core` 的 `GradientButton`，并显式传入原默认渐变、近白文字色、旧主题圆角和禁用态灰色；计划付款列表底部栏的新增按钮已改用 `OutlinedPillButton`。
...
- 账户编辑弹窗的轻量输入模型 `AccountModalAccount` 已迁入 `shared:ui:core`，交易页和旧账户弹窗继续使用同一个窄模型，但不再从 legacy modal 包暴露该数据契约。
...
- 重排弹窗已迁入 `shared:ui:core`，`ReorderModalSingleType` 继续保留原 RecyclerView 拖拽排序、排序号重新计算、完成按钮和弹窗行为；账户、分类、预算和借贷页面不再为了重排入口依赖 legacy modal 包。
- 旧标签弹窗标签列表继续收窄：`ShowTagModal` 不再接收或回传完整 `Tag` 作为公开 UI 契约，改用只含 id 和名称的 `TagModalTag`；编辑交易在业务边界把编辑结果映射回正式 `Tag`。
...
- 金额输入弹窗已迁入 `shared:ui:core`：`AmountModal` / `CalculatorModal` 继续保留原金额键盘、计算器、加减号切换、本地小数分隔符和格式化行为；feature 层不再为了金额输入弹窗导入 legacy modal 包。
