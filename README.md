...
1. 继续替换旧设计兼容 API：下一步优先处理 `com.ivy.wallet.ui.theme.wallet` 包名。
2. 收敛 `shared:ui:legacy` 中仍以旧 Wallet 包名命名的金额/周期展示组件，把通用部分迁到更清晰的 legacy/ui 包名，功能专用组件再下沉到对应 feature。
