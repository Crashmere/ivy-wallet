...
- 基础币种读写进一步收敛为正式 domain 用例：`GetBaseCurrencyUseCase`、`GetBaseCurrencyCodeUseCase`、`SetBaseCurrencyUseCase`。Main、汇率、首页、设置、饼图、计划付款、借贷、编辑交易和首次默认账户预置不再为了基础币种读写直接注入 `CurrencyRepository`；CSV 导入和旧 domain logic 暂时保留仓库依赖，后续随旧账户 mapper/旧统计逻辑单独拆。
