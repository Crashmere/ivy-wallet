...
- app 模块已移除自身不再直接使用的 Ktor、Room、OpenCSV、Keval、RecyclerView 和 AndroidX Security 依赖；源码层已经不再直接引用 data core DAO/repository，只显式依赖仍被默认数据初始化使用的 `shared:data:model`。
